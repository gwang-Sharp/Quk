...
namespace Fisk.EnterpriseManageUtilities.DBUtility
{
    ...
    public  class DbHelperSQL
    {
        ...
        public DbHelperSQL(string conectionstr)
        {
      ...
      ,[TABLE_NAME] from INFORMATION_SCHEMA.TABLES where TABLE_TYPE='BASE TABLE' ";
        ...
        //得到对应表列名
        public DataSet GetColumns(string schemaName, string tableName)
        {
            string sql = @"    select TABLE_SCHEMA,
   TABLE_NAME,
   COLUMN_NAME,
...
from INFORMATION_SCHEMA.COLUMNS ";
            ...
            {
                sql += " where TABLE_SCHEMA='" + schemaName + "' and TABLE_NAME='" + tableName + "' ";
            }
            return Query(sql);
        }
...
#pragma warning restore CS0168 // 声明了变量“ex”，但从未使用过
        ...
        /// <param name="tableName">DataSet结果中的表名</param>
        /// <returns>DataSet</returns>
        public DataSet RunProcedureAllTable(string storedProcName, IDataParameter[] parameters)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                DataSet dataSet = new DataSet();
